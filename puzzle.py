...
class GameGrid(Frame):
    ...
    def key_down(self,event):
        ...
        if key==a.KEY_BACK and len(self.history_matrixs)>1:
            ...
            self.update_grid_cells()
            print('返回上一步:',len(self.history_matrixs))
        elif key in self.commands:
            self.matrixs,done=self.commands[repr(event.char)](self.matrix)
            if done:
                self.matrix=control.add_two(self.matrix)
                self.history_matrixs.append(self.matrix)
                self.update_grid_cells()
                if control.game_state(self.matrix)=='win':
                    self.grid_cells[1][1].configure(text="胜",bg=a.BACKGROUND_COLOR_CELL_EMPTY)
                    self.grid_cells[1][2].configure(text="利",bg=a.BACKGROUND_COLOR_CELL_EMPTY)
                if control.game_state(self.matrix)=='lose':
                    self.grid_cells[1][1].configure(text="失",bg=a.BACKGROUND_COLOR_CELL_EMPTY)
                    self.grid_cells[1][2].configure(text="败",bg=a.BACKGROUND_COLOR_CELL_EMPTY)
